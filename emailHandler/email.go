...
func Email_init() *gomail.Dialer {
	...
	emailUser := set.GetEmailUser()
	emailPassword := set.GetEmailPassword()
	EMAIL_From = set.GetEmailSender()
	EMAIL_Footer = set.GetEmailFooter()
	if EMAIL_Footer == "" {
		EMAIL_Footer = "This is an automated email. Please do not reply!"
	}
	EMAIL_Footer = EMAIL_Footer + "<br><br>--"
	EMAIL_Footer = EMAIL_Footer + fmt.Sprintf("<br><br><small><i>Generated by: %s (%s) on %s</i></small>", set.GetApplicationName(), set.GetServerHost(), time.Now().Format(DATEMSG))
...
func SendEmail(to string, name string, subject string, body string) {
	...
	m.SetAddressHeader(ES_CC, set.GetAdminEmail(), "Admin")
